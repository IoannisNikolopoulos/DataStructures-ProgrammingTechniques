...
Edge createWeightedEdge(Vertex vertex, int cost) {
	Edge edge = createEdge(vertex);
	edge->cost = cost;
	return edge;
}
...
void InsertWeightedEdge(Graph graph, Vertex start, Vertex end, int cost) {
	...
	Edge newEdge = createWeightedEdge(end, cost);
...
void ShowGraph(Graph graph) {
    for (int i = 0; i < graph->vertexCount; i++) {
        ...
        while (edge != NULL) {
            printf(" (end: %d, cost: %d)", edge->endVertex, edge->cost);
            edge = edge->next;
        }
        printf("\n");
    }
...
Vertex* BreadthTopSort(Graph graph) {
	...
	int index = 0;
	Vertex* topSort = malloc(graph->vertexCount * sizeof(*topSort));
...
void ShortestPaths(Graph g, Vertex start) {
	Vertex* sortedVertices = BreadthTopSort(g);

	// Initialize distances
	int* distances = malloc(g->vertexCount * sizeof(*distances));
	for (int i = 0; i < g->vertexCount; i++) {
		if (sortedVertices[i] == start) {
			distances[sortedVertices[i]] = 0;
		} else {
			distances[sortedVertices[i]] = INT_MAX;
		}
	}

	for (int i = 0; i < g->vertexCount; i++) {
		Vertex v = sortedVertices[i];
		if (distances[v] == INT_MAX) continue;

		Edge currentEdge = g->edges[v];
		while (currentEdge != NULL) {
			int newDistance = distances[v] + currentEdge->cost;

			if (distances[currentEdge->endVertex] > newDistance) {
				distances[currentEdge->endVertex] = newDistance;
			}
